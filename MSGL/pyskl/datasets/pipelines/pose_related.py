...
class JointToDisxyC2:
    ...
    def __init__(self, dataset='nturgb+d', target='keypoint'):
        ...
        if self.dataset not in ['nturgb+d', 'openpose', 'coco', 'handmp']:
            ...
            self.pairs = ((0, 1, 2), (1, 2, 5), (2, 3, 5), (3, 4, 5), (4, 0, 5), (5, 0, 6), (6, 7, 9), (7, 8, 9), (8, 0, 9), (9, 0, 10), (10, 11, 13),
                          (11, 12, 13), (12, 0, 13), (13, 0, 14), (14, 15, 17), (15, 16, 17), (16, 0, 17), (17, 0, 18), (18, 19, 1), (19, 20, 1),
                          (20, 0, 1))
    ...
    def __call__(self, results):
        ...
        M, T, V, C = keypoint.shape
        C = 4
        dis = np.zeros((M, T, V, C), dtype=np.float32)
        for v1, v2, v3 in self.pairs:
            dis[..., v1, :2] = keypoint[..., v1, :2] - keypoint[..., v2, :2]
            dis[..., v1, 2:4] = keypoint[..., v1, :2] - keypoint[..., v3, :2]
        results[self.target] = dis
        return results
...
class JointToDiseuc:
    ...
    def __init__(self, dataset='nturgb+d', target='keypoint'):
        self.dataset = dataset
        self.target = target
        if self.dataset not in ['handmp']:
            raise ValueError(
                f'The dataset type {self.dataset} is not supported')
        if self.dataset == 'handmp':
                pass
    def __call__(self, results):
        ...
        for idx in range(V):
            for jdx in range(V):
                dis[..., idx,jdx] = np.linalg.norm(keypoint[..., idx, :2] - keypoint[..., jdx, :2], axis=-1)
        results[self.target] = dis
        return results
...
class JointToDisxyC:
    ...
    def __call__(self, results):
        ...
        for idx in range(V):
            for jdx in range(V):
                dis[..., idx,2*jdx:2*jdx+2] = keypoint[..., idx, :2] - keypoint[..., jdx, :2]
        results[self.target] = dis
        return results
    
class JointToDisxyCMo:
    ...
    def __call__(self, results):
        ...
        for idx in range(V):
            for jdx in range(V):
                dis[:,:T-1, idx,2*jdx:2*jdx+2] = keypoint[:,1:T, idx, :2] - keypoint[:,:T-1, jdx, :2]
        results[self.target] = dis
        return results

class JointToDisxyCNo:
    ...
    def __call__(self, results):
        ...
        for idx in range(V):
            ...
            for jdx in range(V):
                if idx != jdx:
                    dis[..., idx,2*ijdx:2*ijdx+2] = keypoint[..., idx, :2] - keypoint[..., jdx, :2]
                    ijdx = ijdx +1
        results[self.target] = dis
        return results
...
class GenSkeFeat:
    def __init__(self, dataset='nturgb+d', feats=['j'], axis=-1):
        ...
        if 'disxyc' in feats:
            ops.append(JointToDisxyC(dataset=dataset, target='disxyc'))
        if 'disxycmo' in feats:
            ops.append(JointToDisxyCMo(dataset=dataset, target='disxycmo'))
