...
class meca_layer(nn.Module):
    ...
    def forward(self, x):
        ...
        y_avg = self.conv_avg(y_avg.squeeze(-1).transpose(-1, -2)).transpose(-1, -2).unsqueeze(-1)
...
class mseca_layer(nn.Module):
    ...
    def __init__(self, k_size=7):
        super(mseca_layer, self).__init__()
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        self.max_pool = nn.AdaptiveMaxPool2d(1)
        k_size=1
        self.conv_avg1 = nn.Conv1d(1, 1, kernel_size=k_size, padding=(k_size - 1) // 2, bias=False)
        self.conv_max1 = nn.Conv1d(1, 1, kernel_size=k_size, padding=(k_size - 1) // 2, bias=False)
        k_size=3
        self.conv_avg3 = nn.Conv1d(1, 1, kernel_size=k_size, padding=(k_size - 1) // 2, bias=False)
        self.conv_max3 = nn.Conv1d(1, 1, kernel_size=k_size, padding=(k_size - 1) // 2, bias=False)
        k_size=5
        self.conv_avg5 = nn.Conv1d(1, 1, kernel_size=k_size, padding=(k_size - 1) // 2, bias=False)
        self.conv_max5 = nn.Conv1d(1, 1, kernel_size=k_size, padding=(k_size - 1) // 2, bias=False)
        k_size=7
        self.conv_avg7 = nn.Conv1d(1, 1, kernel_size=k_size, padding=(k_size - 1) // 2, bias=False)
        self.conv_max7 = nn.Conv1d(1, 1, kernel_size=k_size, padding=(k_size - 1) // 2, bias=False)
        k_size=1
        self.conv = nn.Conv1d(8, 1, kernel_size=k_size, padding=(k_size - 1) // 2, bias=False)
        self.sigmoid = nn.Sigmoid()
    ...
    def forward(self, x):
        ...
        y_max = self.max_pool(x)
        # Two different branches of ECA module
        y_avg1 = self.conv_avg1(y_avg.squeeze(-1).transpose(-1, -2))
        y_max1 = self.conv_max1(y_max.squeeze(-1).transpose(-1, -2))
        y_avg3 = self.conv_avg3(y_avg.squeeze(-1).transpose(-1, -2))
        y_max3 = self.conv_max3(y_max.squeeze(-1).transpose(-1, -2))
        y_avg5 = self.conv_avg5(y_avg.squeeze(-1).transpose(-1, -2))
        y_max5 = self.conv_max5(y_max.squeeze(-1).transpose(-1, -2))
        y_avg7 = self.conv_avg7(y_avg.squeeze(-1).transpose(-1, -2))
        y_max7 = self.conv_max7(y_max.squeeze(-1).transpose(-1, -2))
        y = torch.cat((y_avg1, y_max1, y_avg3, y_max3, y_avg5, y_max5, y_avg7, y_max7), dim=1)
...
class MSSpatialAttention(nn.Module):
    def __init__(self, kernel_size=7):
        ...
        kernel_size=5
        self.conv5 = nn.Conv2d(2, 1, kernel_size, padding=kernel_size//2, bias=False)
        kernel_size=7
        self.conv7 = nn.Conv2d(2, 1, kernel_size, padding=kernel_size//2, bias=False)
        kernel_size=1
    ...
    def __init__(self,
        ...
        if self.with_res:
            if in_channels != out_channels:
                ...
                self.down = lambda x: x
        self.mssatt = MSSpatialAttention()
    def forward(self, x, A=None):
        """Defines the computation performed at every call."""
        n, c, t, v = x.shape
        res = self.down(x) if self.with_res else 0
        ...
        return self.act(self.mssatt(self.bn(x)) + res)
