...
def mean_class_accuracy(scores, labels):
    ...
    # print(np.column_stack((np.array(scores),np.array(labels))).shape)
    # np.savetxt('PD4Tcls_output.csv', np.column_stack((np.array(scores),np.array(labels))), delimiter=',', fmt='%.4f')
    ...
    # print('混淆矩阵')
    # print(cf_mat)
    # np.savetxt('PRAXISclscm.csv', cf_mat, delimiter=',', fmt='%d')
    # # 测试数
    # print('测试数')
    # print(cls_cnt)
    # # 类别准确率
    # print('类别准确率')
    # print([hit / cnt if cnt else 0.0 for cnt, hit in zip(cls_cnt, cls_hit)])
