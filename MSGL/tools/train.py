...
def parse_args():
    ...
    parser.add_argument('--rank', type=int, default=None, help='find best rank')
    parser.add_argument('--modelsize', type=int, default=None, help='find modelsize')
    parser.add_argument(
        '--deterministic',
        action='store_true',
...
def main():
    ...
    if args.rank is not None:
        # print(cfg.work_dir)
        find_j = cfg.work_dir.split('j')
        find_num = find_j[1].split('_')
        find_num[0] =str (args.rank)
        find_j[1] = '_'.join(find_num)
        cfg.work_dir = 'j'.join(find_j)
        # print(cfg.work_dir)
    # exit(0)
    ...
    if args.modelsize is not None:
        ...
        dummy_input = torch.randn(1, 1, 1, 32, 21, 3).cuda()
        flops, params = profile(model.cuda(), (dummy_input,torch.tensor(1).cuda()))
    ...
    # exit(0)
    #数据参数配置文件读取数据
    # print(build_dataset(cfg.data.train))
    # exit(0)
    datasets = [build_dataset(cfg.data.train)]
    # print('2')
    # print(datasets)
    # dataset_iter = iter(datasets[0])
    # datai = next(dataset_iter)
    # print(datai['keypoint'].shape)
    # exit(0)
