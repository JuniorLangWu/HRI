            ...
            if handpose.multi_hand_landmarks:
                for j, (landmarks, hand_label) in enumerate(zip(handpose.multi_hand_landmarks,handpose.multi_handedness)):
                    ...
                    mp_drawing.draw_landmarks(
                        f, landmarks, mp_hands.HAND_CONNECTIONS)
                    # 添加文本
                    text = anno['frame_dir'] + ':' + str(i) 
                    org = (50, 50)
                    fontFace = cv2.FONT_HERSHEY_SIMPLEX
                    fontScale = 1
                    color = (255, 0, 0)  # 蓝色
                    thickness = 2
                    cv2.putText(f, text, org, fontFace, fontScale, color, thickness)
                    cv2.imshow('MediaPipe Hands', f)
                    if cv2.waitKey(1) & 0xFF == 27:
                        break
                    #检验图片
                    # cv2.imwrite('output_image.jpg', f)
                    # exit(0)
